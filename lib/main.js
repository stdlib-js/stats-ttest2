...
function ttest2( x, y, options ) {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '1Lt8R', x ) );
	}
	if ( !isTypedArrayLike( y ) && !isNumberArray( y ) ) {
		throw new TypeError( format( '1Lt9k', y ) );
	}
	opts = {};
	if ( options ) {
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Lt8V', 'alpha', alpha ) );
	}
	nx = x.length;
	ny = y.length;
	...
	if ( vars === 'equal' ) {
		...
		throw new Error( format( '1Lt4S', 'variance', [ 'equal', 'unequal' ].join( '", "' ), vars ) );
	}
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '1Lt4S', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	out = {};
